...
public class ResultOfSkServiceImpl implements IResultOfSkService {
	...
	public Map<String, String> queryResult(String fpqqlsh, String fplx) {
		logger.info("** 订单查询   **发票请求流水号：" + fpqqlsh + ",发票类型：" + fplx);
		Map<String, String> map = new HashMap<String, String>();
		Kpxx kpxx = resultDao.queryResult( fpqqlsh, fplx);
		
		if(null == kpxx){
			logger.info("** 订单查询   **发票请求流水号：" + fpqqlsh + ",发票类型：" + fplx + "查询为null");
			return null;
		}else if("026".equals(kpxx.getFplx())){//电子票
			if("1".equals(kpxx.getFpzt())){
				logger.info("** 订单查询    **发票请求流水号：" + fpqqlsh + ",发票类型：" + fplx + "查询为fpzt:1");
				map.put("returnCode", "2000");
				map.put("returnMsg", "电子开票正在处理中");
				return map;
			}else if("-1".equals(kpxx.getFpzt())){
				logger.info("** 订单查询    **发票请求流水号：" + fpqqlsh + ",发票类型：" + fplx + ",开具成功，查询为fpzt:" + kpxx.getFpzt());
			...
			if("1".equals(kpxx.getFpzt())){
				...
				logger.info("** 订单查询     **发票请求流水号：" + fpqqlsh +  ",发票类型：" + fplx + "查询为fpzt:2");
				map.put("returnCode", "2000");
