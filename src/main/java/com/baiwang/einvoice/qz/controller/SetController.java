...
public class SetController {
	...
	@RequestMapping("quNsrsbh")
	@ResponseBody
	public Map<String, Object> quNsrsbh(HttpServletRequest request){
		...
		if(null == user || !user.getCzydm().equals("admin")){
			...
			List<String> nsrsbh = userService.getNsrsbh();
			map.put("code",	"0");
	...
	@ResponseBody
	public Map<String, Object> initsetting(String nsrsbh, HttpServletRequest request){
		Map<String, Object> map = new HashMap<>();
		User user = (User)request.getSession().getAttribute("user");
		if(null == user || !user.getCzydm().equals("admin")){
			logger.warn("系统设置：用户未登陆");
			...
			logger.info("用户：" + user.getCzydm() + "加载系统设置");
			SkConfig config = service.initsetting(nsrsbh);
			map.put("code",	"0");
